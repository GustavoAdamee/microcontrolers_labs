...
; - Altera a frequência do barramento usando o PLL a 80MHz
; - Configura o Systick para utilizar delays precisos multiplos de 1ms

; Editado por:
; Guilherme Peron	- 15/03/2018
; DaLuz				- 26/11/2024
...
PLL_Init
    ...
    MOV32	R1, SYSCTL_RSCLKCFG_R				    		; R1 = SYSCTL_RSCLKCFG_R (ponteiro)
    ...
    MOV32	R1, SYSCTL_MOSCCTL_R  	         	    		; R1 = SYSCTL_MOSCCTL_R (pointer)
    LDR 	R0, [R1]                			    		; R0 = [R1] (value)
    ...
    MOV32	R1, SYSCTL_RIS_R      	                		; R1 = SYSCTL_RIS_R (pointer)
PLL_Init_step3loop
    LDR 	R0, [R1]                                		; R0 = [R1] (value)
    ...
    ;    no offset 0x0B0.
    MOV32 	R1, SYSCTL_RSCLKCFG_R	                		; R1 = SYSCTL_RSCLKCFG_R (pointer)
    LDR 	R0, [R1]     	                        		; R0 = [R1] (value)
    ...
    MOV32 	R1, SYSCTL_DSCLKCFG_R                 			; R1 = SYSCTL_DSCLKCFG_R (pointer)
    LDR 	R0, [R1]                               			; R0 = [R1] (value)
...
SYSCLK EQU (FXTAL/(Q+1)/(N+1))*(MINT+MFRAC/1024)/(PSYSDIV+1)
    MOV32 	R1, SYSCTL_PLLFREQ0_R                 			; R1 = SYSCTL_PLLFREQ0_R (ponteiro)
    LDR 	R0, [R1]      	                        		; R0 = [R1] (valor)
    MOV32 	R3, SYSCTL_PLLFREQ0_MFRAC_M		        		; R3 = SYSCTL_PLLFREQ0_MFRAC_M (máscar)
    BIC 	R0, R0, R3         	                    		; R0 = R0&~SYSCTL_PLLFREQ0_MFRAC_M (limpar o campo MFRAC)
    MOV32 	R3, (MFRAC<<SYSCTL_PLLFREQ0_MFRAC_S)  			; R3 = (MFRAC<<SYSCTL_PLLFREQ0_MFRAC_S) (valor deslocado)
    ADD 	R0, R0, R3                             			; R0 = R0 + (MFRAC<<SYSCTL_PLLFREQ0_MFRAC_S) (configurar MFRAC como definido acima)
    MOV32 	R3, SYSCTL_PLLFREQ0_MINT_M            			; R3 = SYSCTL_PLLFREQ0_MINT_M (máscara)
    ...
    MOV32 	R1, SYSCTL_PLLFREQ1_R                 			; R1 = SYSCTL_PLLFREQ1_R (ponteiro)
    LDR 	R0, [R1]                               			; R0 = [R1] (value)
    ...
    MOV32 	R1, SYSCTL_MEMTIM0_R                  			; R1 = SYSCTL_MEMTIM0_R (ponteiro)
    LDR 	R0, [R1]                               			; R0 = [R1] (valor)
    MOV32 	R3, 0x03EF03EF                        			; R3 = 0x03EF03EF (máscara)
    BIC 	R0, R0, R3                             			; R0 = R0&~0x03EF03EF (limpar os campos EBCHT, EBCE, EWS, FBCHT, FBCE, e FWS fields)
    MOV32 	R2, SYSCLK                            			; R2 = (FXTAL/(Q+1)/(N+1))*(MINT+MFRAC/1024)/(PSYSDIV+1)
    MOV32 	R3, 120000000                          			; R3 = 80,000,000 (value)
...
PLL_Init_step7slow                             				; 16 MHz < SysClk <= 40 MHz: "slow"
    MOV32 	R3, 16000000                          			; R3 = 16,000,000 (value)
...
PLL_Init_step7done
    ...
    MOV32 	R1, SYSCTL_PLLSTAT_R                  			; R1 = SYSCTL_PLLSTAT_R (pointer)
...
PLL_Init_step8done
    ...
    MOV32 	R1, SYSCTL_RSCLKCFG_R                 			; R1 = SYSCTL_RSCLKCFG_R (ponteiro)
    LDR 	R0, [R1]                               			; R0 = [R1] (valor)
    MOV32 	R3, SYSCTL_RSCLKCFG_PSYSDIV_M         			; R3 = SYSCTL_RSCLKCFG_PSYSDIV_M (mascara)
...
; Modifica: R0, R1
SysTick_Init
	MOV32 	R1, NVIC_ST_CTRL_R								; R1 = &NVIC_ST_CTRL_R (ponteiro)
	MOV 	R0, #0 											; desabilita Systick durante a configuração
	STR 	R0, [R1]										; escreve no endereço de memória do periférico
	MOV32 	R1, NVIC_ST_RELOAD_R 							; R1 = &NVIC_ST_RELOAD_R (pointeiro)
	MOV32 	R0, 0x00FFFFFF; 								; valor máximo de recarga 2^24 ticks
	STR 	R0, [R1] 										; escreve no endereço de memória do periférico o NVIC_ST_RELOAD_M
	MOV32 	R1, NVIC_ST_CURRENT_R 							; R1 = &NVIC_ST_CURRENT_R (ponteiro)
	MOV 	R0, #0 											; qualquer escrita no endereço NVIC_ST_CURRENT_R o limpa
	STR 	R0, [R1] 										; limpa o contador
	MOV32 	R1, NVIC_ST_CTRL_R 								; habilita o SysTick com o clock do core
...
; Modifica: R0, R1 e R3
SysTick_Wait
	MOV32 	R1, NVIC_ST_RELOAD_R 							; R1 = &NVIC_ST_RELOAD_RSUB R0 (ponteiro)
	SUB 	R0, #1                          				; Ajustar Contangem com o HW
	STR 	R0, [R1] 										; delay-1, número de contagens para esperar
	MOV32 	R1, NVIC_ST_CTRL_R 								; R1 = &NVIC_ST_CTRL_R
SysTick_Wait_loop
...
SysTick_Wait1ms_loop					
	MOV32 	R0, DELAY1MS 									; R0 = DELAY1MS (número de ticks para contar 1ms)
